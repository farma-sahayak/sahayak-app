...
import '../repository/schemes_repository.dart';
...
class _SchemesPageViewState extends State<_SchemesPageView> {
              ...
              children: [
                ...
                BlocBuilder<SchemesBloc, SchemesState>(
                  builder: (context, state) {
                    ...
                    final schemesToShow = state.filteredSchemes.isNotEmpty || 
                        _hasActiveFilters() 
                    ...
                    return Column(
                      children: [
                        ...
                        // Schemes
                        ...schemesToShow.map((scheme) => SchemeCard(
                          scheme: scheme,
                          onApply: () => _handleApplyForScheme(context, scheme),
                          onTrack: () => _handleTrackApplication(context, scheme),
                          onInfo: () => _handleShowSchemeInfo(context, scheme),
                          onPlayAudio: () => _handlePlayAudio(context, scheme),
                          onDownloadBrochure: () => _handleDownloadBrochure(context, scheme),
                        )),
                      ],
                    );
                  },
  ...
  void _handleTrackApplication(BuildContext context, Scheme scheme) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Tracking ${scheme.title} application'),
      ),
    );
  }
  ...
  void _handlePlayAudio(BuildContext context, Scheme scheme) {
    if (scheme.audioGuideUrl != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Playing audio guide...')),
      );
    }
  }
  ...
  void _handleDocumentsList(BuildContext context) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Opening documents list...')),
    );
  }
  ...
  void _handleForms(BuildContext context) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Opening forms...')),
    );
  }
