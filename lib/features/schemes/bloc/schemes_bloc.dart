...
class SchemesBloc extends Bloc<SchemesEvent, SchemesState> {
  ...
  SchemesBloc({
    required SchemesRepository repository,
  ...
  Future<void> _onLoadBenefitsOverview(
    ...
    try {
      final benefitsOverview = await _repository.getBenefitsOverview(event.farmerId);
  ...
  List<Scheme> _applyFilters(List<Scheme> schemes, SchemesState currentState) {
    ...
    if (currentState.selectedCategoryFilter != null) {
      filtered = filtered
          .where((scheme) => scheme.category == currentState.selectedCategoryFilter)
          .toList();
    }
