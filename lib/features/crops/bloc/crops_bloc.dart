...
class CropsBloc extends Bloc<CropsEvent, CropsState> {
  ...
  Future<void> _onLoadCrops(LoadCrops event, Emitter<CropsState> emit) async {
    ...
    try {
      ...
      final financialDashboard = await repository.getFinancialDashboard('farmer_123');
  ...
  Future<void> _onRefreshCrops(RefreshCrops event, Emitter<CropsState> emit) async {
    try {
      final crops = await repository.getCrops();
      final financialDashboard = await repository.getFinancialDashboard('farmer_123');
      emit(CropsLoaded(crops: crops, financialDashboard: financialDashboard));
  ...
  Future<void> _onUpdateCropMilestone(
    ...
    try {
      await repository.updateMilestone(event.cropId, event.milestoneId, event.isCompleted);
  ...
  Future<void> _onUploadCropImage(
    ...
    emit(ImageUploading());
    try {
      final imageUrl = await repository.uploadCropImage(event.cropId, event.imagePath);
